...
try:
    ...
    from .global_hotkey_listener import (
...
def write_current_config_to_file():
    ...
    config_data = {
        ...
        "pathignore_patterns": session.get("pathignore_patterns", []),
        "pathignore_input_text": session.get("pathignore_input_text", ""),
    ...
    try:
        ...
        logger.info("Updated config file: %s", CONFIG_FILE_PATH)
    except Exception as e:
        logger.error(f"Error writing config file {CONFIG_FILE_PATH}: {e}")


def is_valid_path(base_path, check_path):
    """Return whether an existing path is contained by the selected root."""
    try:
        base = os.path.realpath(os.path.abspath(base_path))
        candidate = os.path.realpath(os.path.abspath(check_path))
        return (
            os.path.exists(candidate) and os.path.commonpath([base, candidate]) == base
        )
...
def generate_file_tree(
    ...
    try:
        ...
        try:
            ...
            for item in items:
                ...
                if not should_ignore_path(
                    item_path, root_path, respect_gitignore, pathignore_patterns
                ):
        ...
        for item in sorted_items:
            ...
            # Hidden items logic
            if item.startswith("."):
                if item in [".gitignore", ".pathignore"]:
                    pass
                elif os.path.isdir(item_absolute_path) and show_hidden_folders:
                    pass
                else:
                    continue
            ...
            escaped_relative_path = escape(relative_path, quote=True)
            escaped_absolute_path = escape(item_absolute_path, quote=True)

            if os.path.isdir(item_absolute_path):
                html += '<li class="directory">'
                html += f'<input type="checkbox" class="dir-checkbox" data-path="{escaped_relative_path}"> '
                html += '<span class="expand-button"><i class="fas fa-caret-right"></i></span>'
                html += f'<span class="directory-name" data-path="{escaped_relative_path}">{escaped_item}</span>'
                html += '<ul style="display: none;">'
                html += generate_file_tree(
                    root_path,
                    item_absolute_path,
                    respect_gitignore,
                    respect_pathignore,
                ...
                if tree_type == "main" and file_ext in SPECIAL_MEDIA_EXTS:
                    continue
                if tree_type == "special" and file_ext not in SPECIAL_MEDIA_EXTS:
                    continue

                if tree_type == "special":
                    html += '<li class="file special-file">'
                    html += f'<input type="checkbox" class="special-file-checkbox" data-path="{escaped_absolute_path}"> '
                    html += f'<span class="file-name" data-ext="{escape(file_ext, quote=True)}">{escaped_item}</span>'
                    html += f'<span class="file-size badge badge-secondary">{file_size_str}</span>'
                    html += '<div class="btn-group btn-group-xs ml-2">'
                    html += f'<button class="btn btn-outline-info copy-native-btn" data-path="{escaped_absolute_path}" title="Copy to clipboard"><i class="fas fa-copy"></i></button>'
                    html += f'<button class="btn btn-outline-success open-file-btn" data-path="{escaped_absolute_path}" title="Open file"><i class="fas fa-external-link-alt"></i></button>'
                    html += f'<button class="btn btn-outline-secondary locate-file-btn" data-path="{escaped_absolute_path}" title="Show in File Explorer"><i class="fas fa-folder-open"></i></button>'
                    html += "</div></li>"
                else:
                    html += '<li class="file">'
                    html += f'<input type="checkbox" class="file-checkbox" data-path="{escaped_relative_path}" data-size="{file_size}"> '
                    html += f'<span class="file-name" data-ext="{escape(file_ext, quote=True)}">{escaped_item}</span>'
...
def index():
    ...
    if request.method == "POST":
        try:
            ...
            show_hidden_folders = data.get("show_hidden_folders", False)
            # Get raw pathignore text from JS and parse it
            pathignore_input_text = data.get(
                "pathignore_input_text", session.get("pathignore_input_text", "")
            )  # Use session as fallback
            parsed_pathignore_patterns = (
                parse_ignore_patterns(pathignore_input_text)
                if respect_pathignore
                else []
            )
            ...
            if "path" in data:  # Navigation
                ...
                new_absolute_path = os.path.normpath(
                    os.path.join(absolute_root, relative_path)
                )
            ...
            session["show_hidden_folders"] = show_hidden_folders
            session["pathignore_patterns"] = (
                parsed_pathignore_patterns  # Store parsed list
            )
            session["pathignore_input_text"] = (
                pathignore_input_text  # Store raw text for UI
            )
            ...
            file_tree_html_main = generate_file_tree(
                new_absolute_path,
                new_absolute_path,
            ...
            file_tree_html_special = generate_file_tree(
                new_absolute_path,
                new_absolute_path,
                respect_gitignore=respect_gitignore,
                respect_pathignore=respect_pathignore,
                pathignore_patterns=parsed_pathignore_patterns,
                show_hidden_folders=show_hidden_folders,
                tree_type="special",
            )
            return jsonify(
                {"fileTree": file_tree_html_main, "specialTree": file_tree_html_special}
            )
        except Exception as e:
        ...
        if absolute_root and os.path.exists(absolute_root):
            ...
            show_hidden = session.get("show_hidden_folders", False)
            file_tree_html = generate_file_tree(
                absolute_root,
                absolute_root,
                show_hidden_folders=show_hidden,
                tree_type="main",
            )
            special_file_tree_html = generate_file_tree(
                absolute_root,
                absolute_root,
                show_hidden_folders=show_hidden,
                tree_type="special",
            ...
            special_file_tree_html = (
                "<ul><li class='empty-prompt'>No special files found.</li></ul>"
            )
        ...
        return render_template(
            ...
            special_file_tree_html=special_file_tree_html,
        )
    ...
    try:
        ...
        if data is None or "enabled" not in data:
            return jsonify(
                {
                    "success": False,
                    "error": 'Invalid request data. "enabled" field missing.',
                }
            ), 400
        ...
        logger.info(
            f"Global hotkey setting updated to: {'Enabled' if is_enabled else 'Disabled'}"
        )
        return jsonify(
            {
                "success": True,
                "message": f"Global hotkey has been {'enabled' if is_enabled else 'disabled'}. Restart app for changes to take effect.",
            }
        )
    except Exception as e:
        ...
        return jsonify(
            {"success": False, "error": f"An internal error occurred: {str(e)}"}
        ), 500
...
def get_file_content():
    try:
        ...
        if file_size > max_size:
            return jsonify(
                {
                    "content": f"(File is too large ({format_size(file_size)}) to display inline)"
                }
            )
        ...
        if content is None:
            return jsonify(
                {"content": "(Binary or non-text file - content not displayed)"}
            )
...
def generate_output():
    try:
        ...
        max_size_kb = int(
            data.get("maxSizeKB", session.get("max_size_kb", 100))
        )  # Use session as fallback
        ...
        result = generate_output_content(
            selected_files, absolute_root, max_size_kb, include_binary=True
        )
...
def api_browse():
    ...
    respect_gitignore_arg = (
        request.args.get("respect_gitignore", "true").lower() == "true"
    )
    respect_pathignore_arg = (
        request.args.get("respect_pathignore", "false").lower() == "true"
    )
    ...
    parsed_pathignore_patterns_for_browse = (
        parse_ignore_patterns(pathignore_patterns_str.replace(",", "\n"))
        if respect_pathignore_arg
        else []
    )
    ...
    file_tree_data = generate_file_tree_json(
        path,
        path,
        respect_gitignore_arg,
        respect_pathignore_arg,
    ...
    return jsonify({"file_tree": file_tree_data, "root_path": path})


@app.route("/api/copy_file_native", methods=["POST"])
def api_copy_file_native():
    try:
        data = request.get_json()
        paths = data.get("paths")
        absolute_root = session.get("absolute_root")

        if not paths or not absolute_root:
            return jsonify({"error": "Invalid request parameters"}), 400

        if os.name != "nt":
            return jsonify(
                {"error": "Native file copy is currently available on Windows only"}
            ), 501
        ...
        for file_path in paths:
            ...
            if is_valid_path(absolute_root, abs_path) and os.path.isfile(abs_path):
                valid_paths.append(abs_path)

        if not valid_paths:
            return jsonify({"error": "No valid files found on system to copy"}), 404

        import subprocess

        formatted_paths = ["'{}'".format(p.replace("'", "''")) for p in valid_paths]
        paths_str = ", ".join(formatted_paths)
        cmd = [
            "powershell",
            "-NoProfile",
            "-Command",
            "Set-Clipboard -Path {}".format(paths_str),
        ]
        res = subprocess.run(cmd, capture_output=True, text=True)

        if res.returncode != 0:
            return jsonify(
                {"error": "Windows could not copy the selected file(s)"}
            ), 500

        if len(valid_paths) == 1:
            message = f"Copied {os.path.basename(valid_paths[0])} to the clipboard"
        else:
            message = f"Copied {len(valid_paths)} files to the clipboard"

        return jsonify({"message": message})
    except Exception as e:
        logger.error(f"Error executing native copy: {e}")
        return jsonify({"error": f"Selection copying error: {str(e)}"}), 500


@app.route("/api/open_file_native", methods=["POST"])
def api_open_file_native():
    try:
        data = request.get_json()
        file_path = data.get("path")
        absolute_root = session.get("absolute_root")

        if not file_path or not absolute_root:
            return jsonify({"error": "Invalid request parameters"}), 400

        if os.name != "nt":
            return jsonify(
                {"error": "Native file opening is currently available on Windows only"}
            ), 501

        file_path = os.path.abspath(file_path)
        if not is_valid_path(absolute_root, file_path) or not os.path.isfile(file_path):
            return jsonify({"error": "Access denied or file not found"}), 403

        os.startfile(file_path)

        return jsonify({"message": f"Opening {os.path.basename(file_path)}"})
    except Exception as e:
        logger.error(f"Error opening file: {e}")
        return jsonify({"error": str(e)}), 500


@app.route("/api/locate_file_native", methods=["POST"])
def api_locate_file_native():
    try:
        data = request.get_json()
        file_path = data.get("path")
        absolute_root = session.get("absolute_root")
        ...
        if os.name != "nt":
            return jsonify(
                {
                    "error": "File Explorer integration is currently available on Windows only"
                }
            ), 501
        ...
        return jsonify(
            {"message": f"Showing {os.path.basename(file_path)} in File Explorer"}
        )
    except Exception as e:
        logger.error(f"Error locating file: {e}")
        return jsonify({"error": str(e)}), 500
...
def api_select():
    ...
    if not absolute_root:
        return jsonify(
            {"error": "Root path not set. Use /api/browse with set_as_root=true first."}
        ), 400
    ...
    if in_directory:
        ...
        if os.path.isdir(subdirectory_path):
            ...
            return jsonify(
                {"error": f'Specified directory "{in_directory}" not found'}
            ), 400
    ...
    for root, dirs, files in os.walk(search_root):
        for file in files:
            ...
            if any(
                relative_path.lower().endswith(ext.lower())
            ...
            if path_patterns and not any(
                fnmatch.fnmatch(relative_path, pattern) for pattern in path_patterns
            ):
                continue
...
def api_generate():
    ...
    result = generate_output_content(
        selected_files, max_size_kb, absolute_root, include_binary_files
    )

    response = {"files_txt": result["files_txt"], "stats": result["stats"]}
...
def api_reset():
    ...
    return jsonify({"message": "Selection state reset successfully"})


# Helper functions for the API
def generate_file_tree_json(
    root_path,
    current_path,
    respect_gitignore=True,
    ...
    result = {
        ...
        "path": os.path.relpath(current_path, root_path).replace("\\", "/")
    ...
    try:
        ...
        for item in items:
            ...
            if (
                not show_hidden
                and item.startswith(".")
                and item not in [".gitignore", ".pathignore"]
            ):
                continue
            ...
            if not should_ignore_path(
                item_path, root_path, respect_gitignore, combined_patterns
            ):
                filtered_items.append(item)
...
def api_global_trigger_generate_and_copy():
    try:
        ...
        respect_gitignore = data.get("respect_gitignore", True)
        respect_pathignore = data.get("respect_pathignore", True)
        pathignore_patterns = data.get(
            "pathignore_patterns", []
        )  # Expecting parsed patterns
        max_size_kb = int(data.get("max_size_kb", 100))

        if (
            not absolute_root
            or not os.path.exists(absolute_root)
            or not os.path.isdir(absolute_root)
        ):
            return jsonify(
                {"error": "Invalid or missing absolute_root path in request."}
            ), 400

        # Use the core function to collect files
        combined_patterns = (
            pathignore_patterns if respect_pathignore and pathignore_patterns else None
        )
        all_files_in_root = collect_files(
            absolute_root, respect_gitignore, combined_patterns
        )

        if not all_files_in_root:
            return jsonify(
                {
                    "error": "No files found in the specified root after applying ignore rules."
                }
            ), 400

        # Generate content using the core helper function
        # Pass include_binary=False as a default, can be made configurable if needed
        output_data = generate_output_content(
            all_files_in_root, absolute_root, max_size_kb, include_binary=False
        )
        ...
        if not selected_files_from_hotkey:
            ...
            logger.warning(
                "No 'selected_files' in hotkey payload, attempting to process all files in root."
            )
            combined_patterns = (
                pathignore_patterns
                if respect_pathignore and pathignore_patterns
            ...
            if not all_files_in_root:
                return jsonify(
                    {
                        "error": "No files found in the specified root after applying ignore rules (fallback)."
                    }
                ), 400
            ...
            logger.info(
                f"Global trigger processing specific files: {selected_files_to_process}"
            )

        # Generate content using the core helper function with the determined list of files
        output_data = generate_output_content(
            selected_files_to_process, absolute_root, max_size_kb, include_binary=False
        )
        ...
        combined_content += (
            f"Binary Files (content not included): {stats.get('binary_files', 0)}\n"
        )

        logger.info(
            f"Global trigger: Generated content for {len(selected_files_to_process)} files from {absolute_root}"
        )
        return jsonify(
            {
...
def api_browse_folder():
    """Opens a native OS folder selection dialog and returns the chosen path."""
    try:
        ...
        root.update()  # Process events so the window is properly hidden

        folder_path = filedialog.askdirectory(
            title="Select Project Folder", parent=root
        )
        ...
        logger.error(f"Error opening folder browser dialog: {e}")
        logger.error(traceback.format_exc())
        return jsonify(
            {"success": False, "error": f"Could not open folder dialog: {str(e)}"}
        ), 500
...
def api_update_current_selection():
    try:
        ...
        logger.info(
            f"Updated last_selected_files in session and config: {len(selected_files)} files."
        )
        return jsonify(
            {
                "message": f"Successfully updated current selection with {len(selected_files)} files."
            }
        ), 200
    except Exception as e:
...
if __name__ == "__main__":
    ...
    listener_logger.info("Keyboard listener started.")
    app.run(host="127.0.0.1", port=5023, debug=False, use_reloader=False)
