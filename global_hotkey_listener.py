...
def read_config():
    ...
    try:
        if not os.path.exists(CONFIG_FILE_PATH):
            logger.warning(
                f"Config file not found at {CONFIG_FILE_PATH}. Waiting for app to create it."
            )
            return None
        with open(CONFIG_FILE_PATH, "r", encoding="utf-8") as f:
            config = json.load(f)
            logger.info("Loaded FilesToAI configuration.")
            return config
...
def trigger_generate_and_copy():
    ...
    logger.info("Hotkey %s triggered.", HOTKEY)
    current_time = time.time()

    if current_time - last_triggered_time < DEBOUNCE_SECONDS:
        logger.info(
            f"Debounce: Hotkey '{HOTKEY}' triggered too soon. Ignoring. Last: {last_triggered_time:.2f}, Current: {current_time:.2f}, Diff: {current_time - last_triggered_time:.2f}s"
        )
        return

    logger.info(
        f"Processing hotkey '{HOTKEY}' press. Attempting to trigger generation."
    )
    ...
    enable_global_hotkey = config.get(
    ...
    if not payload["selected_files"]:
        logger.warning(
            "No 'last_selected_files' found in config. The API will likely process all files in the root."
        )
        # The API endpoint has a fallback to process all files if 'selected_files' is empty or missing.

    logger.info("Requesting output for %s.", absolute_root)

    try:
        response = requests.post(
            API_ENDPOINT, json=payload, timeout=60
        )  # 60 second timeout
        ...
        if response_data.get("combined_content"):
            ...
            logger.error(
                f"API response missing 'combined_content'. Response: {response_data}"
            )

    except requests.exceptions.ConnectionError:
        logger.error(
            f"Connection to Flask app ({FLASK_APP_URL}) failed. Is the app running?"
        )
        ...
        logger.error(
            f"Error with clipboard: {e}. Is a copy/paste mechanism installed (e.g., xclip or xsel on Linux)?"
        )
    except Exception as e:
...
def setup_keyboard_listener():
    ...
    def test_hotkey_callback():
        logger.info(
            f"--- TEST HOTKEY ({TEST_HOTKEY}) PRESSED SUCCESSFULLY! Keyboard library is hooking keys. ---"
        )
    ...
    enable_hotkey_at_startup = (
        True  # Default to true if config is missing or key is missing
    )
    ...
    if enable_hotkey_at_startup:
        try:
            ...
            logger.info(
                f"Successfully registered MAIN hotkey: {HOTKEY} (enabled at startup)"
            )
        except Exception as e:
            ...
            logger.error(
                "Try running the main Flask app (app.py) with administrator/root privileges."
            )
            # No return here, as the app might still run without the hotkey if other parts are fine.
    else:
        logger.info(
            f"MAIN hotkey ({HOTKEY}) was NOT registered because 'enable_global_hotkey' is false in config at startup."
        )
    ...
    if not os.path.exists(CONFIG_FILE_PATH):
        logger.warning(
            f"Config file {CONFIG_FILE_PATH} not found. Creating a default one."
        )
        logger.warning(
            "Please run the FilesToAI web application once to populate it with your desired settings."
        )
        default_config = {
    ...
    logger.info(
        f"Keyboard listener setup complete. Hotkeys '{HOTKEY}' (and test '{TEST_HOTKEY}' if enabled) are active."
    )
...
if __name__ == "__main__":
    ...
    if HOTKEY in keyboard.all_hotkeys():  # Check if main hotkey was actually registered
        logger.info(
            f"Listener is active (standalone mode). Press '{HOTKEY}' to trigger. Press 'Esc' to stop this script."
        )
        try:
            keyboard.wait("esc")
        ...
        logger.error(
            f"Main hotkey {HOTKEY} failed to register in standalone mode. Exiting."
        )
