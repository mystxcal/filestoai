...
def gitignore_pattern_to_regex(pattern):
    ...
    if pattern.startswith("/"):
        pattern = (
            "^" + pattern[1:]
        )  # Match from beginning of path, remove leading slash
    elif pattern.startswith("\\/"):  # check for escaped slash second (less common)
        pattern = (
            "^" + pattern[2:]
        )  # Match from beginning of path, remove escaped slash
...
def _gitignore_rules_for_path(path, root_path):
    """Yield ignore rules from the root down to the path's parent directory."""
    ...
    for ignore_dir in directories:
        ignore_path = os.path.join(ignore_dir, ".gitignore")
        for pattern in parse_ignore_file(ignore_path):
            yield pattern, ignore_dir


def should_ignore_path(
    path, root_path, respect_gitignore=True, custom_patterns=None, debug=False
):
    """
    ...
    if respect_gitignore:
        rule_sets.extend(_gitignore_rules_for_path(path, root_path))

    if custom_patterns:
        rule_sets.extend((pattern, root_path) for pattern in custom_patterns)

    if not rule_sets:
        if debug:
            logger.debug("No ignore patterns to check.")
        return False

    for pattern, rule_root in rule_sets:
        negate = False
        if pattern.startswith("!"):
            negate = True
            pattern = pattern[1:]

        candidate_path = os.path.relpath(path, rule_root).replace("\\", "/")
        regex_pattern = gitignore_pattern_to_regex(pattern)
        if debug:
            logger.debug(
                "Testing pattern %s against %s (regex: %s)",
                pattern,
                candidate_path,
                regex_pattern,
            )

        if not re.search(regex_pattern, candidate_path):
            continue
    ...
    if debug:
        logger.debug(f"Final ignore status for {rel_path}: {ignored}")
    return ignored


def collect_files(
    root_path, respect_gitignore=True, custom_patterns=None, show_hidden_folders=False
):
    """
    ...
    for dirpath, dirnames, filenames in os.walk(root_path):
        ...
        for i, dn in enumerate(dirnames):
            if dn.startswith(".") and not show_hidden_folders:
                continue
            filtered_dirnames.append((dn, abs_dirnames[i]))

        dirnames[:] = [
            dn
            for dn, abs_dn in filtered_dirnames
            if not should_ignore_path(
                abs_dn, root_path, respect_gitignore, custom_patterns
            )
        ]
        ...
        for filename in filenames:
            ...
            if not should_ignore_path(
                file_path, root_path, respect_gitignore, custom_patterns
            ):
...
def generate_output_content(
    selected_files, root_path, max_size_kb=100, include_binary=False
):
    """
    ...
    for relative_file_path in selected_files:
        ...
        try:
            ...
            if file_size > max_size_bytes:
                files_txt_content += (
                    f"=== File: {os.path.basename(relative_file_path)} ===\n"
                )
